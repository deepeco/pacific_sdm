...
CURRENT_PATH_PATTERN = os.path.join(DATA_PATH, 'current')
...
DATA_PREFIXES = ('tmin', 'tmax', 'tmean', 'prec', 'wind')

DATA_PATTERNS = dict()
for pref in DATA_PREFIXES:
    DATA_PATTERNS.update({
                          pref.upper() + str(k):  {'filename':  os.path.join(CURRENT_PATH_PATTERN, pref, COM_PREFIX % pref + '{:1d}'.format(k)) + DATA_EXT } for k in range(1, 13)
                          })

BIO_PREFIX = 'bio_' #trim_wc2_bio_30s
DATA_PATTERNS.update({
                       'BIO' + str(k):  {'filename':  os.path.join(CURRENT_PATH_PATTERN, 'bio', BIO_PREFIX + '{:1d}'.format(k)) + DATA_EXT } for k in range(1, 20)
                })

# ------------- Data from the future --------------------
FUTURE_POSTFIX = '.tif'
FUTURE_PATH_PATTERN = DATA_PATH + 'future/%s/%s/%s/%s/'
...
PAST_POSTFIX = ('.bil', '.tif')
PAST_PATH_PATTERN = DATA_PATH + 'past/%s/%s/%s'
PAST_FILE_PATTERN = '%s%s%s%s%s'

PAST_PERIOD = ['lgm', 'mid']
PAST_VARS = ['bi', 'tn', 'tx', 'pr']
PAST_MODELS = ['cc', 'mr',]
PAST_MONTHS = map(str, range(1, 13))
for model, period, var, month, postfix in product(PAST_MODELS,
                                          PAST_PERIOD,
                                          PAST_VARS,
                                          PAST_MONTHS,
                                          PAST_POSTFIX):
    path_pat = PAST_PATH_PATTERN % (model, period, var)
    file_pat = PAST_FILE_PATTERN % (model, period, var, month, postfix)
    
    
    if os.path.exists(os.path.join(path_pat, file_pat)) and postfix == '.bil':
        DATA_PATTERNS.update({
            PAST_VARS_MAPPING[var]+month+'_' + model + period: {'filename': os.path.join(path_pat, file_pat)}
                            })
    elif os.path.exists(os.path.join(path_pat, file_pat)) and postfix == '.tif':
        if (PAST_VARS_MAPPING[var]+month+'_' + model + period) not in DATA_PATTERNS:
            DATA_PATTERNS.update({
                PAST_VARS_MAPPING[var]+month+'_' + model + period: {'filename': os.path.join(path_pat, file_pat)}
                                })
    if var == 'bi':
        for m in range(13, 20):
            file_pat = PAST_FILE_PATTERN % (model, period, var, m, postfix)
            if os.path.exists(os.path.join(path_pat, file_pat)) and postfix == '.bil':
                DATA_PATTERNS.update({
                ...
                if (PAST_VARS_MAPPING[var]+str(m) + '_' + model+period) not in DATA_PATTERNS:
                    DATA_PATTERNS.update({
                        PAST_VARS_MAPPING[var]+str(m) + '_' + model+period: {
                        'filename': os.path.join(path_pat, file_pat)}
                    })
